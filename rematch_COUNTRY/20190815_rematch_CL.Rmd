---
title: "sample-matching_IPUMS-NQ_CL, wave 16 'Blitz'"
author: "Maita Schade"
date: "Aug 15, 2019"
...
target <- Reduce(function(dtf1, dtf2) {merge(dtf1, dtf2, #[names(dtf1)[-grep("targetId|X",names(dtf1))]]  [names(dtf2)[-grep("targetId|X",names(dtf2))]]
...
apply(responded[responded$wave>=10,c(NQ_id,"SAMPID","wave")],1,
      function(r){
...
```{r}
# for (i in 1:5){
#   for (c in responded[is.na(responded$SAMPID),][[NQ_id]]){
#     n = dim(waves[[i]][waves[[i]][[paste0("gp_codigo.",i)]]==c,])[1]
#     if (n>0){print(c)}
#   }
# }
```
...
```{r}
age_q <- quantile(census$age,prob = seq(0,1,1/3))
alldata['age_group'] <- as.integer(cut(alldata$age,breaks = age_q, include.lowest = TRUE))
alldata$age_group[is.na(alldata$age_group)] <- 3
```
