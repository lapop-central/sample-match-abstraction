...
responsefile <- paste0('C:/Users/schadem/Box Sync/LAPOP Shared/working documents/maita/Coordination/IDB Online Crime/Data processing/Data/IADB+Argentina+Questionnaire+-+Netquest+v2_October+28%2C+2019_08.59.csv')
#file of not yet sent IDs
# excludefile <- paste0('C:/Users/schadem/Box Sync/LAPOP Shared/working documents/maita/Coordination/Noam Argentina Panel/Data processing/Data/APE_2019_sept7v2_October 7, 2019_09.01.csv')
...
head(target.pre)
```
If using the pre-election sample, load it from the first wave...
```{r}
...
# 
nrow(target)
previous[sampleId%in%target$sampleId]
```
...
if (length(list.files(path=paste0(datadir,"panel/"), pattern = "wave"))>0){
  ...
  if(sum(!target.select$sampleId%in%target$sampleId)==0){ #if all IDs in selected are in sample
