...
responsefile <- paste0('C:/Users/schadem/Box Sync/LAPOP Shared/working documents/maita/Coordination/Noam Argentina Panel/Data processing/Data/APE_2019_sept7v2_October 3, 2019_11.23.csv')
...
if (!is.null(responsefile)){
  ...
  if (sum(duplicated(responded.wide$sampleId))!= 0){
    print("Alert! Duplicated sampleIds in responses")
    print(sum(duplicated(responded.wide$sampleId)))
    }
  ...
  target.pruned <- target
    ...
    # Check that no sampleId's are duplicated:
  if (sum(duplicated(target.pruned$sampleId))!= 0){
    print("Alert! Duplicated sampleIds in target!")
    print(sum(duplicated(target.pruned$sampleId)))
    }
